    ...
    fetch(apiUrl)
        ...
        .then((data) => {
            ...
            var searchInput3 = document.getElementById("searchInput3");
      		var searchTerm3 = searchInput3.value.toLowerCase(); // 검색어를 소문자로 변환하여 대소문자 구분 없이 검색
            ...
            for (let i = 0; i < items.length; i++) {
				...
				if (title.toLowerCase().includes(searchTerm3)){
...
// "더 보기" 버튼 선택
var loadMoreResultsButton3 = document.getElementById("loadMoreResults3");

// "더 보기" 버튼 클릭 시 결과 추가 로드
loadMoreResultsButton3.addEventListener("click", loadMoreResults3);

var currentPage = 2; // 초기 페이지


function loadMoreResults3() {
    let selectedContentTypeId = contentTypeIdSelect3.value;
    let selectedAreaCode = areaCodeSelect3.value;
