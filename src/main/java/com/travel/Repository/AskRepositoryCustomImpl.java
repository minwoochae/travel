...
public class AskRepositoryCustomImpl implements AskRepositoryCustom{
	...
	private BooleanExpression askTitleLike(String searchQuery) {
		return StringUtils.isEmpty(searchQuery)? 
				null : QAskBoard.askBoard.askTitle.like("%" + searchQuery + "%"); 
	}
	...
	public Page<MainAskDto> getMainAskPage(AskSearchDto askSearchDto, Pageable pageable) {
		...
		List<MainAskDto> content = queryFactory.select(
				new QMainAskDto(
						askBoard.id, askBoard.askTitle, askBoard.askContent)
				)
				.from(askBoardImg)
				.join(askBoardImg.askBoard, askBoard)
				.where(askTitleLike(askSearchDto.getSearchQuery()))
				.orderBy(askBoard.id.desc())
				.offset(pageable.getOffset())
				.limit(pageable.getPageSize())
				.fetch();
